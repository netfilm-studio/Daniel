...
function updateLightbox(title) {
  ...
  setTimeout(() => {
    setImageWithFallback(img, lightboxPhotos[lightboxIdx]);
    img.onload = () => {
      img.style.opacity = '1';
...
function buildInfoModal() {
  ...
  if (detailsEl) {
    ...
    html += `<p><strong>Она:</strong> ${КЛИЕНТ.она}</p>`;
    html += `<p><strong>Он:</strong> ${КЛИЕНТ.он}</p>`;
...
function initAll() {
  ...
  if (typeof BG !== 'undefined') {
    ...
    if (isVideo) {
      ...
      if (wbg) wbg.style.backgroundImage = "url('" + BG + "')";
      if (wbgVideo) wbgVideo.style.display = 'none';
      if (infoImg) { infoImg.src = BG; infoImg.style.display = 'block'; }
